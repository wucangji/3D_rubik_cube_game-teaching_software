...
class Singmaster:public solutions    
{
	private:
		...
		vector<bool> checkCorrectCube(cube cub,char face,int state)
		{
			...
			if(state==0)
			{
				...
				for(int j=0;j<4;j++)
				{
					switch(j)
					{
						case 0:{x2=1;y2=0;z2=2;
						        x1=2;y1=0;z1=1;
						        x0=2;y0=0;z0=0;break;}
						case 1:{x2=0;y2=-1;z2=2;
							x1=0;y1=-2;z1=1;
							x0=0;y0=-2;z0=0;break;}
						case 2:{x2=-1;y2=0;z2=2;
						        x1=-2;y1=0;z1=1;
						        x0=-2;y0=0;z0=0;break;}
						case 3:{x2=0;y2=1;z2=2;
						        x1=0;y1=2;z1=1;
						        x0=0;y0=2;z0=0;break;}
					}
					if(cub.rubik[0+2][0+2][2+2]==cub.rubik[x2+2][y2+2][z2+2]
					   &&cub.rubik[x1+2][y1+2][z1+2]==cub.rubik[x0+2][y0+2][z0+2])
					{
		...
		string makeCross(cube& cub,int startface)
		{
			...
			//cout<<"the center color:"<<center<<endl;
			int faceColor=getFaceCenter(cub,'U',i);
			//cout<<"the face color:"<<faceColor<<endl;
			//cout<<"at this time i="<<i<<endl;
			switch(i)
			{
				case 0:{cub.Yr();trueTableRot("Yr",cTablePoint);toRet+="y' ";break;}
				case 1:{cub.Y2();trueTableRot("Y2",cTablePoint);toRet+="y2 ";break;}
				case 2:{cub.Y();trueTableRot("Y",cTablePoint);toRet+="y ";break;}
				case 3:{break;}
				default:break;
			}
			//cout<<"makeCross: point 1.0"<<endl;
			
			string rightCube=findCube(cub,center,faceColor);
			//cout<<"Find the right cube position:"<<rightCube<<endl;
			int indic=0;
			if(rightCube[0]=='U'&&indic==0)
			{
				if(rightCube[1]=='F')
				{
					indic=1;
			...
			if((rightCube[1]=='D'||rightCube[0]=='D')&&indic==0)
			{
				if(rightCube[1]=='F'||rightCube[0]=='F')
				{
					indic=1;
					cub.D();
					toRet+="D ";
				}
				if(rightCube[1]=='L'||rightCube[0]=='L')
				{
					indic=1;
					cub.D2();
					toRet+="D2 ";
				}
				if(rightCube[1]=='B'||rightCube[0]=='B')
				{
					indic=1;
					cub.Dr();
					toRet+="D' ";
			...
			if((rightCube[0]=='F'||rightCube[1]=='F')&&indic==0)
			{
			...
			if((rightCube[0]=='B'||rightCube[1]=='B')&&indic==0)
			{
				if(rightCube[0]=='L'||rightCube[1]=='L')
				{
					indic=1;
		...
		//=========NOW, I HAVE MADE THE CUBE FROM EVERY POSSIBLE POSITION TO TWO BASIC ONES.====/ 	
		...
		string makeOppoCross(cube& cub,int startface)
		{
			...
			*/
			...
			{
				if(upFace[1][2]==center&&upFace[2][1]==center)
				{
					cub.Y2();
					toRet+="y2 ";
					/*
				...
				if(upFace[1][0]==center&&upFace[2][1]==center)
				{
					cub.Y();
					toRet+="y ";
				}	
		...
		{
			//cout<<"makeOppoFace started!"<<endl;
			string toRet;
			string *toRetPoint=&toRet;
			switchFaceUp(&cub,startface,toRetPoint);
			cub.X2();
